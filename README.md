# Guide

## Home 
<img src="./gh-static/home.png" width="800"/>

## RSVP
<img src="./gh-static/rsvp.png" width="800"/>

1. Clone the project `git clone https://github.com/pseudoClone/Event-Manager.git`
2. Run `npm install`. Make sure you have `npm` installed.
3. Prepare your database(if you are feeling lucky, try a self hosted database and not Neon).
4. Follow [env example file](./env.example) and populate it as per instructions
> Note: Shadcn/ui has a tendency to break. Make sure to delete all the unnecessary styles and themes unless you like ugly appearance.